...
async def build_context_score(db: AsyncSession, event_id: str) -> dict:
    """Run all context-score components concurrently and return results."""
    injury, props, pace, cross_market = await asyncio.gather(
        get_injury_context(db, event_id),
        get_player_props_context(db, event_id),
        get_pace_context(db, event_id),
        get_cross_market_context(db, event_id),
    )
    return {
        "event_id": event_id,
